...
interface PenghasilanCardProps {
    data: {
        id: number;
        namaFile: string;
...
const PenghasilanCard: React.FC<PenghasilanCardProps> = ({ data }) => {
    const navigate = useNavigate();
    const [showConfirm, setShowConfirm] = useState(false);
    const [showSuccess, setShowSuccess] = useState(false);

    const handleDetailClick = () => {
        navigate(`/laporan-penghasilan/${data.id}`);
    };

    const handleDeleteClick = async () => {
        try {
            const response = await apiInstance.delete(`penghasilan/${data.id}`);
            if (response.status === 200) {
                setShowConfirm(false);
                setShowSuccess(true);
                setTimeout(() => {
                    setShowSuccess(false);
                    navigate("/laporan-penghasilan");
                }, 2000);
        ...
        <>
            <div className="relative overflow-hidden bg-white/70 backdrop-blur-md border border-gray-200 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 p-6 group">
                ...
                <div className="relative z-10">
                    <h2 className="text-2xl font-bold text-gray-800 hover:text-gray-900 transition-all duration-300">{data.namaFile}</h2>
                    ...
                    <div className="mt-4 text-sm text-gray-700 space-y-2">
                        <div>
                            <span className="font-medium text-gray-800">Nama Pengeluaran:</span>{" "}
                            <span className="text-gray-600">{data.namaPengeluaran}</span>
                        </div>
                        <div>
                            <span className="font-medium text-gray-800">Nominal Pengeluaran:</span>{" "}
                            <span className="text-gray-600">Rp {data.nominalPengeluaran.toLocaleString()}</span>
                        </div>
                        <div>
                            <span className="font-medium text-gray-800">Total Keuntungan:</span>{" "}
                            <span className="text-green-600">Rp {data.hasilTotal.toLocaleString()}</span>
                        </div>
                    </div>
                    <div className="card-button-modal flex flex-row justify-between">
                    <button
                        onClick={handleDetailClick}
                        className="justify-center w-1/2 mt-4 mr-2 flex button-detail items-center px-4 py-2 bg-gradient-to-r  text-white text-sm font-medium rounded-full hover:scale-105 active:scale-95 transition-transform shadow-md"
                    >
                        Lihat Detail
                    </button>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-lg p-6 max-w-sm w-full text-center">
                        ...
                        <p className="text-gray-600 mb-6">
                            Apakah Anda yakin ingin menghapus file <strong>{data.namaFile}</strong>?
                        </p>
...
export default PenghasilanCard;
